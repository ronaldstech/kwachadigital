...
const FEATURES = [
    { icon: <Zap size={14} className="text-yellow-500" />, text: 'Draft Chapters 2, 3, 4, and 5' },
    { icon: <ShieldCheck size={14} className="text-green-500" />, text: 'Advanced Research Analysis' },
    { icon: <Check size={14} className="text-indigo-500" />, text: 'Unlimited Word & PDF Exports' },
    { icon: <Trophy size={14} className="text-purple-500" />, text: 'Full Academic Reference List' },
];
...
const HDR = {
    ...
    success: { title: '🎉 Project Unlocked!', sub: 'You are now on Pro!' },
...
const SubscriptionModal = ({ isOpen, onClose, onUpgrade, userName, userEmail, dissertationId, userId }) => {
    ...
    useEffect(() => {
        if (step === 'history' && dissertationId) {
            setLoadingHist(true);
            fetchDissertationTransactions(dissertationId)
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            const result = await processPayment({
                ...
                dissertationId: dissertationId || 'unknown',
